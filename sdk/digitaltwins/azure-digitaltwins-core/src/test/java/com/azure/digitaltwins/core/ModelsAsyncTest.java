...
public class ModelsAsyncTest extends ModelsTestBase {
        ...
        createModelsRunner(asyncClient, (modelsList) -> {
            StepVerifier.create(asyncClient.createModels(modelsList))
                .assertNext(createdModel -> {
                    createdModels.addAll(createdModel);
                })
                .verifyComplete();
        });
    ...
    public void createModelThrowsIfModelAlreadyExists(HttpClient httpClient, DigitalTwinsServiceVersion serviceVersion) {
        ...
        final String wardModelId = UniqueIdHelper.getUniqueModelId(TestAssetDefaults.wardModelId, asyncClient);
    ...
    private static void createModelsRunner(DigitalTwinsAsyncClient asyncClient, Consumer<List<String>> createModelsTestRunner) {
        String buildingModelId = UniqueIdHelper.getUniqueModelId(TestAssetDefaults.buildingModelId, asyncClient);
        String floorModelId = UniqueIdHelper.getUniqueModelId(TestAssetDefaults.floorModelId, asyncClient);
        String hvacModelId = UniqueIdHelper.getUniqueModelId(TestAssetDefaults.hvacModelId, asyncClient);
        String wardModelId = UniqueIdHelper.getUniqueModelId(TestAssetDefaults.wardModelId, asyncClient);
